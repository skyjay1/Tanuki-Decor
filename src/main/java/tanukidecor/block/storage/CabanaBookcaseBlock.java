...
public class CabanaBookcaseBlock extends HorizontalMultiblock implements EntityBlock {

    public CabanaBookcaseBlock(Properties pProperties) {
        super(MultiblockHandler.MULTIBLOCK_2X1X1,
                HorizontalMultiblock.createEWShapeBuilder(CabanaDresserBlock.SHAPE_EAST, CabanaDresserBlock.SHAPE_WEST),
                pProperties);
    }
    ...
    public BlockEntity newBlockEntity(BlockPos pPos, BlockState pState) {
        if(pPos.equals(getDelegatePos(pState, pPos))) {
            return TDRegistry.BlockEntityReg.CABANA_BOOKCASE.get().create(pPos, pState);
        }
